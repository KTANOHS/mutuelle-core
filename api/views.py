# api/views.py - VERSION CORRIGÉE SANS IMPORT CIRCULAIRE ET AVEC GESTION DES IMPORTS
import sys
import os
from datetime import timedelta

# =============================================================================
# 1. CONFIGURER DJANGO EN PREMIER (avant tout import)
# =============================================================================
...
if not DJANGO_CONFIGURED:
    # Configurer Django minimalement pour les imports
    os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'mutuelle_core.settings')
    try:
        import django
        django.setup()
        print("✅ Django configuré pour l'importation")
    except Exception as e:
        print(f"⚠️  Impossible de configurer Django: {e}")

# =============================================================================
# 2. DÉFINIR api_health EN PREMIER (fonction simple sans dépendances)
# =============================================================================

def api_health(request):
    """Endpoint de santé de l'API - version minimale"""
...
# =============================================================================
# 3. IMPORTER LES AUTRES MODULES (maintenant que Django est configuré)
# =============================================================================

try:
    # Maintenant nous pouvons importer les modules Django/DRF
    from django.http import JsonResponse
    from django.utils import timezone
    from django.conf import settings
    from rest_framework.decorators import api_view
    
    # Décorez api_health avec @api_view
    api_health = api_view(['GET'])(api_health)
    
    # Importer le reste de DRF
    ...
    print("✅ Tous les imports DRF réussis")
    
except ImportError as e:
    print(f"⚠️  Erreur d'importation DRF: {e}")
    ...
    class Response:
        def __init__(self, data, status=None): pass
...
# =============================================================================
# 4. IMPORTS DES MODÈLES AVEC GESTION D'ERREURS
# =============================================================================
...
try:
    ...
    print(f"⚠️  Erreur d'importation des modèles: {str(e)[:100]}")
...
# =============================================================================
# 5. VIEWSETS (avec vérification de la disponibilité)
# =============================================================================
...
class TypeSoinViewSet(viewsets.ModelViewSet):
    ...
    serializer_class = TypeSoinSerializer if MODELS_AVAILABLE else None
    permission_classes = [IsAuthenticated] if 'IsAuthenticated' in globals() else []
    
    @property
    def filter_backends(self):
        if DjangoFilterBackend and SearchFilter and OrderingFilter:
...
class SoinViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['post'], permission_classes=[IsAuthenticated] if 'IsAuthenticated' in globals() else [])
...
class PrescriptionViewSet(viewsets.ModelViewSet):
    ...
    serializer_class = PrescriptionSerializer if MODELS_AVAILABLE else None
    permission_classes = [IsAuthenticated] if 'IsAuthenticated' in globals() else []
    
    @property
    def filter_backends(self):
        if DjangoFilterBackend and SearchFilter:
...
class MedecinViewSet(viewsets.ReadOnlyModelViewSet):
    def get_queryset(self):
        if not MODELS_AVAILABLE:
            return []
        return Medecin.objects.filter(actif=True).select_related('user')
    
    serializer_class = MedecinSerializer if MODELS_AVAILABLE else None
    permission_classes = [IsAuthenticated] if 'IsAuthenticated' in globals() else []
    filter_backends = [SearchFilter, OrderingFilter] if SearchFilter and OrderingFilter else []
...
class MembreViewSet(viewsets.ModelViewSet):
    def get_queryset(self):
        if not MODELS_AVAILABLE:
            return []
        return Membre.objects.filter(statut='actif')
    
    serializer_class = MembreSerializer if MODELS_AVAILABLE else None
    permission_classes = [IsAuthenticated] if 'IsAuthenticated' in globals() else []
...
class BonPriseEnChargeViewSet(viewsets.ModelViewSet):
    def get_queryset(self):
        if not MODELS_AVAILABLE:
            return []
        return BonPriseEnCharge.objects.all()
    
    serializer_class = BonPriseEnChargeSerializer if MODELS_AVAILABLE else None
    permission_classes = [IsAuthenticated] if 'IsAuthenticated' in globals() else []
...
class PaiementViewSet(viewsets.ModelViewSet):
    ...
    serializer_class = PaiementSerializer if MODELS_AVAILABLE else None
    permission_classes = [IsAuthenticated] if 'IsAuthenticated' in globals() else []
    
    @property
    def filter_backends(self):
        if DjangoFilterBackend and OrderingFilter:
...
class OrdonnanceViewSet(viewsets.ModelViewSet):
    ...
    serializer_class = OrdonnanceSerializer if MODELS_AVAILABLE else None
    permission_classes = [IsAuthenticated] if 'IsAuthenticated' in globals() else []
...
class StatistiquesAPIView(generics.GenericAPIView):
    ...
    def get(self, request):
        if not MODELS_AVAILABLE:
            return Response({'error': 'Modèles non disponibles'}, status=status.HTTP_503_SERVICE_UNAVAILABLE)

...
class RealTimeMedecinMonitor:
    ...
    def estimer_revenus_semaine(self):
        ...
        revenus_consultations = consultations_semaine * self.medecin.tarif_consultation
...
def lister_medecins_actifs():
    ...
    for medecin in medecins:
        ...
        print(f"   • {medecin.nom_complet} - {consultations_count} consultations")
...
if __name__ == "__main__":
    ...
    # Option 1: Lister les médecins disponibles
    medecins = lister_medecins_actifs()
    
    if medecins.exists():
        # Prendre le premier médecin actif
        premier_medecin = medecins.first()
        medecin_id = premier_medecin.id
        
        print(f"\n🎯 Analyse du Dr {premier_medecin.nom_complet} (ID: {medecin_id})")
        
        # Option 2: Rapport rapide
        print("\n1. 📋 RAPPORT RAPIDE")
        analyser_medecin(medecin_id)
        
        # Option 3: Démarrer le monitoring
        print("\n2. 🔄 MONITORING TEMPS RÉEL")
        reponse = input("   Démarrer le monitoring? (o/n): ")
        
        if reponse.lower() in ['o', 'oui', 'y', 'yes']:
            monitor = RealTimeMedecinMonitor(medecin_id)
            monitor.start_monitoring(interval_minutes=2)  # 2 min pour les tests
    else:
        print("❌ Aucun médecin actif trouvé dans la base de données")
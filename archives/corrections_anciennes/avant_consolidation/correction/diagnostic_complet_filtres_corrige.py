...
try:
    ...
    def diagnostic_complet_filtres():
        ...
        if response.status_code == 200:
            ...
            # 4. Vérification du contexte (corrigée)
            print("\n4. 📋 CONTEXTE FOURNI:")
            if hasattr(response, 'context') and response.context is not None:
                context = response.context
                patients_in_context = len(context.get('patients', []))
                maladies_in_context = len(context.get('maladies', []))
                print(f"   👥 Patients dans contexte: {patients_in_context}")
                print(f"   🩺 Maladies dans contexte: {maladies_in_context}")
            else:
            ...
            patient_count = len(patient_options) - 1  # -1 pour l'option vide
            print(f"   👥 Options patients: {patient_count}")
            
            # Compter les options maladies
            maladie_options = re.findall(r'<option value="\d+"', content)
            maladie_count = len(maladie_options) - 1  # -1 pour l'option vide
            print(f"   🩺 Options maladies: {maladie_count}")
        ...
        if patients_count == 0:
            print("   ⚠️  CRÉATION DE PATIENTS REQUISE")
            print("   💡 Exécutez: python creer_donnees_test_accompagnement.py")
            
        if maladies_count == 0:
            print("   ⚠️  CRÉATION DE MALADIES REQUISE")
            print("   💡 Exécutez: python creer_donnees_test_accompagnement.py")
        ...
        if patients_count > 0 and maladies_count > 0:
            ...
            print("   🚨 LES FILTRES NE FONCTIONNERONT PAS SANS DONNÉES!")
            print("   📋 Exécutez le script de création de données maintenant")

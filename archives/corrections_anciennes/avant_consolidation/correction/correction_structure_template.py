...
<div class="container-fluid py-4">
    ...
    <!-- SECTION DES CONVERSATIONS - STRUCTURE CORRIGÉE -->
                ...
                <div class="conversation-item border rounded p-3 mb-3 bg-light">
                    <div class="d-flex justify-content-between align-items-start">
                        <div class="flex-grow-1">
                            ...
                            <!-- AUTRE PARTICIPANT -->
                            <div class="mb-2">
                                <strong class="text-dark">Avec:</strong>
                                {% for participant in conversation.participants.all %}
                                    {% if participant != request.user %}
                                        ...
                                        <small class="text-muted">
                                            ({{ participant.get_full_name|default:"Pas de nom complet" }})
                                        </small>
                                    {% endif %}
                                {% endfor %}
                            </div>
                            ...
                            <!-- DERNIER MESSAGE -->
                            <div class="mb-2">
                                <strong class="text-dark">Dernier message:</strong>
                                {% with dernier_message=conversation.messages.last %}
                                    {% if dernier_message %}
                                        ...
                                        <small class="text-muted">
                                            (par {{ dernier_message.expediteur.username }})
                                        </small>
                                    {% else %}
                                        <em class="text-muted">Aucun message dans cette conversation</em>
                                    {% endif %}
                                {% endwith %}
                            </div>
                            ...
                            <div class="mb-2">
                                ...
                                <span class="badge bg-info me-1">{{ conversation.total_messages }} message(s)</span>
                                <span class="badge bg-danger">{{ conversation.nb_messages_non_lus }} non lu(s)</span>
                            </div>
                            ...
                            <div>
                                <small class="text-muted">
                                    ...
                                    {% if conversation.derniere_activite %}
                                        <strong>{{ conversation.derniere_activite|timesince }}</strong>
                                    {% else %}
            ...
            {% else %}
                ...
                <div class="text-center py-5">
                    ...
                    <p class="text-muted mb-4">
                        Vous n'avez pas encore de conversation dans la base de données.
                    </p>
...
'''
    ...
    if os.path.exists(template_path):
        backup_path = template_path + '.backup_structure_cassee'
        os.rename(template_path, backup_path)
        print(f"✅ Ancien template cassé sauvegardé: {backup_path}")
    
    # Écrire le nouveau template corrigé
    with open(template_path, 'w') as f:
        f.write(template_corrige)
    
    print("✅ TEMPLATE COMPLÈTEMENT RECONSTRUIT AVEC SUCCÈS !")
    print("🎯 La structure a été corrigée :")
    print("   - {% extends %} est maintenant la première ligne")
    print("   - Tout le contenu est dans {% block content %}")
    print("   - Structure claire et organisée")
    print("🌐 Testez maintenant: http://127.0.0.1:8000/communication/")

if __name__ == "__main__":
    corriger_structure_template()
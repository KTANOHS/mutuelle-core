#!/usr/bin/env python
"""
Script de validation spécifique pour mututlle_core
"""
...
def setup_django():
    """Configure Django"""
    try:
        os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'mututlle_core.settings')
        django.setup()
        return True
...
def check_corrections():
    ...
    print("🔍 VÉRIFICATION DES CORRECTIONS - MUTUTLLE_CORE")
    print("=" * 50)
    ...
    checks = {
        ...
        "mututlle_core/models.py existe": os.path.exists('mututlle_core/models.py'),
        "Méthode __str__ dans mututlle_core Session": False,
    }
    ...
    # Vérifie mututlle_core/models.py
    if checks["mututlle_core/models.py existe"]:
        models_content = open('mututlle_core/models.py').read()
        checks["Méthode __str__ dans mututlle_core Session"] = 'class Session' in models_content and 'def __str__' in models_content
    ...
    if not all_passed:
        ...
        if not checks["mututlle_core/models.py existe"]:
            print("- Créer mututlle_core/models.py avec modèle Session")
    ...
    if all_passed:
        ...
        print("\\n🚀 Le projet mututlle_core est prêt pour la production!")
    else:
        print("⚠️  Certaines corrections sont manquantes")
        print("💡 Exécutez: ./fix_mututlle.sh pour les appliquer")

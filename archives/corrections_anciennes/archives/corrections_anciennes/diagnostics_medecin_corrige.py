# diagnostics_medecin_corrige.py
import os
import django
...
def diagnostiquer_medecin_complet():
    ...
    if not any('medecin:' in str(url.get('full_name', '')) for url in all_medecin_urls):
        print("PROBLEME: Aucune URL trouvée dans le namespace 'medecin'")
        print("\nSOLUTIONS:")
        print("1. Créer le fichier medecin/urls.py avec:")
        print("   from django.urls import path")
        print("   from . import views")
        print("   app_name = 'medecin'")
        print("   urlpatterns = [")
        print("       path('dashboard/', views.dashboard, name='dashboard'),")
        print("       path('login/', views.login_medecin, name='login'),")
        print("       path('profil/', views.profil, name='profil'),")
        print("       path('consultations/', views.liste_consultations, name='consultations'),")
        print("       path('ordonnances/', views.liste_ordonnances, name='ordonnances'),")
        print("   ]")
        print("\n2. Inclure dans le urls.py principal:")
        print("   path('medecin/', include('medecin.urls', namespace='medecin')),")
    else:
        print("✓ La structure de base des URLs medecin semble correcte")

if __name__ == "__main__":
    diagnostiquer_medecin_complet()